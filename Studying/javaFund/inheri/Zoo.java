...
//		System.out.println("이것은 " + animal.type);
		...
		Animal dog = new Dog("댕댕이", "리트리버");
		System.out.println(dog);

		Animal cat = new Cat("야옹이", "오드아이");
...
//		다형성 : 상속관계에서 instance를 발생 시, super 타입을 하위 instance를 생성하는 형태를 말함
//		이렇게 하면, 타입은 super 로 해당 인스턴스의 외형은 자식으로, 내부에는 부모 인스턴스가 생성이 되어짐
...
//		final : 고정의 의미를 담고 있음.
//		클래스앞에 선언되면 상속 금지, 메서드 앞에 선언되면 오버라이드 금지, 변수 앞에 선언되면 값 변경 금지, static final 로 선언되면
//		상수로 표현되는 의미로 사용됨.
...
//		일반적으로 사용하는 이유는 특정 추상 or 인터페이스의 하위 객체를 한번만 만들어서 사용하고 버릴 때임
